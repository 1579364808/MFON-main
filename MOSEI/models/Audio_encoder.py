...
class AudioEncoder(nn.Module):
    ...
    该模块用于将原始音频特征编码为高级语义表示，主要用于：
    1. 音频模态的预训练
    2. 作为TVA融合模型中的音频编码组件
    3. 在融合训练中作为冻结的教师模型提供知识蒸馏

    架构：
    - 线性投影层：将74维音频特征映射到768维统一空间
    - Transformer编码器：使用自注意力机制建模音频序列的时序依赖关系
    - 输出：取第一个时间步的表示作为整个音频序列的句子级表示
    """
    ...
    def __init__(self, config):
        ...
        self.audio_seq_len = config.MOSEI.downStream.audio_seq_len          # 500: 音频序列长度
        self.audio_nhead = config.MOSEI.downStream.audio_nhead              # 8:   多头注意力的头数
        self.audio_tf_num_layers = config.MOSEI.downStream.audio_tf_num_layers  # 2: Transformer层数
        self.attn_dropout = config.MOSEI.downStream.audio_drop_out          # 0.0: 注意力dropout率
        self.attn_mask = config.MOSEI.downStream.audio_attn_mask            # False: 是否使用注意力掩码

        # ========== 网络层定义 ==========
        # 线性投影层：将原始音频特征(74维)映射到统一的编码器维度(768维)
        # 这样可以与文本和视觉特征在同一空间中进行处理
        self.proj_a = nn.Linear(self.audio_fea_dim, self.encoder_fea_dim)

        # Transformer编码器：用于建模音频序列的时序依赖关系
        # 使用自注意力机制捕获不同时间步之间的相关性
        self.trans_encoder_a = TransformerEncoder(
            embed_dim=self.encoder_fea_dim,         # 768: 嵌入维度
            num_heads=self.audio_nhead,             # 8:   多头注意力头数
            layers=self.audio_tf_num_layers,        # 2:   Transformer层数
    ...
    def forward(self, audio):
        ...
        Args:
            audio: 输入音频特征 [batch_size, seq_len, audio_fea_dim]
                  形状为 [bs, 500, 74]

        Returns:
            last_h_a: 音频的句子级表示 [batch_size, encoder_fea_dim]
                     形状为 [bs, 768]

        处理流程：
        1. 线性投影：74维 → 768维
        2. 维度转换：适配Transformer输入格式
        3. Transformer编码：建模时序依赖关系
        4. 提取表示：取第一个时间步作为句子级表示
        """

        # 步骤1: 线性投影 - 将音频特征映射到统一的编码器维度
        # 输入: [bs, 500, 74] → 输出: [bs, 500, 768]
        ...
        if type(h_as) == tuple:
            h_as = h_as[0]  # 只取输出，忽略注意力权重

        # 步骤4: 提取句子级表示 - 取第一个时间步的输出作为整个序列的表示
        # [500, bs, 768] → [bs, 768]
        last_h_a = h_as[0]   # 相当于 h_as[0, :, :]
...
# 删除AudioPretrain类 - 不再需要单模态预训练
    ...
    该模型用于音频模态的自监督预训练，主要目标是：
    1. 学习音频特征的语义表示：通过情感回归任务让模型理解音频内容的情感含义
    2. 为融合模型提供初始化：预训练的编码器作为TVA融合模型的初始化权重
    3. 知识蒸馏的教师模型：在融合训练中作为冻结的教师模型指导学习

    架构组成：
    - AudioEncoder: 音频编码器，将74维音频特征编码为768维语义表示
    ...
    def __init__(self, config, encoder_fea_dim=None):
        ...
        # ========== 核心组件初始化 ==========
        # 音频编码器：将原始音频特征(74维×500序列)编码为语义表示(768维)
        ...
        self.classifier = BaseClassifier(
            ...
            name='AudioRegClassifier'                                      # 分类器名称标识
        )
    ...
    def forward(self, audio, label, return_loss=True, device=None):
        ...
        处理流程：
        1. 音频编码：通过AudioEncoder将音频特征编码为语义表示
        ...
        # 步骤1: 音频编码 - 将原始音频特征编码为高级语义表示
        # 输入: [bs, 500, 74] → 输出: [bs, 768]
    ...
    def save_model(self, name='best_loss'):
        ...
        Args:
            name: 保存文件的前缀名，默认为'best_loss'
                 最终文件名格式：{name}_audio_encoder.pt 和 {name}_audio_decoder.pt

        保存文件：
        - 编码器权重：{model_path}/{name}_audio_encoder.pt
        - 分类器权重：{model_path}/{name}_audio_decoder.pt
        """
        # 构建保存路径
        ...
        # 分别保存编码器和分类器的状态字典
        torch.save(self.encoder.state_dict(), encoder_path)      # 保存音频编码器权重
    ...
    def load_model(self, name='best_loss', module=None):
        ...
        Args:
            name: 加载文件的前缀名，默认为'best_loss'
                 对应的文件名：{name}_audio_encoder.pt 和 {name}_audio_decoder.pt
            module: 指定加载的模块，可选值：
                   - 'encoder': 只加载音频编码器
                   - 'decoder': 只加载回归分类器
        ...
        使用场景：
        - module='encoder': 在TVA融合模型中初始化音频编码器
        - module='decoder': 在特定分类任务中复用分类器
        - module='all': 完整的音频预训练模型推理或继续训练
        """
        # 构建加载路径
        encoder_path = self.model_path + name + '_audio_encoder.pt'
        decoder_path = self.model_path + name + '_audio_decoder.pt'
