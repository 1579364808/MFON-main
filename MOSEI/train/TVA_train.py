...
def TVA_train_fusion(config, metrics, seed, train_data, valid_data, test_data):
    ...
    # ========== AtCAF K-means 初始化检查 ==========
    need_atcaf_init = config.MOSEI.downStream.use_atcaf_plugin and not model.atcaf_plugin.vision_dict.initialized
    ...
    for epoch in range(1, total_epoch + 1):
        ...
        for batch_data in bar:
            try:
                ...
                # AtCAF损失（如果启用）
                if getattr(model.config.MOSEI.downStream, 'use_atcaf_plugin', False):
                ...
                # HCL损失（只显示启用的）
                if getattr(model.config.MOSEI.downStream, 'enable_hcl', False):
                    lambda1 = getattr(model.config.MOSEI.downStream, 'lambda_scl', 0.0)
                    lambda2 = getattr(model.config.MOSEI.downStream, 'lambda_iamcl', 0.0)
                    lambda3 = getattr(model.config.MOSEI.downStream, 'lambda_iemcl', 0.0)
                ...
                # Focal Loss（如果启用）
                if getattr(model.config.MOSEI.downStream, 'enable_aux_cls', False):
            ...
            # ===== 混合对比损失 =====
            if getattr(model.config.MOSEI.downStream, 'enable_hcl', False):
                alpha = getattr(model.config.MOSEI.downStream, 'contrastive_alpha', 0.5)
                lambda1 = getattr(model.config.MOSEI.downStream, 'lambda_scl', 0.0)
                lambda2 = getattr(model.config.MOSEI.downStream, 'lambda_iamcl', 0.0)
                lambda3 = getattr(model.config.MOSEI.downStream, 'lambda_iemcl', 0.0)
            ...
            if cls_logits is not None:
                ...
                fa = getattr(model.config.MOSEI.downStream, 'focal_alpha', None)
                if fa is None:
                    ...
                    alpha_pos = float(fa)
                gamma = getattr(model.config.MOSEI.downStream, 'focal_gamma', 2.0)
                focal_w = getattr(model.config.MOSEI.downStream, 'focal_weight', 0.1)
            ...
            if torch.is_tensor(atcaf_loss_raw) and atcaf_loss_raw.item() > 0:
                counterfactual_weight = model.config.MOSEI.downStream.counterfactual_weight

                # warm-up: 前 atcaf_warmup_epochs 轮，将权重线性从 0 -> counterfactual_weight
                warmup_epochs = getattr(model.config.MOSEI.downStream, 'atcaf_warmup_epochs', 0)
