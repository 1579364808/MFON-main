...
class TVA_fusion(nn.Module):
    def __init__(self, config):
        ...
        if self.use_learnable_vectors:
            # 支持独立配置可学习向量长度
            learnable_vision_len = getattr(config.MOSI.downStream, 'learnable_vision_len', self.vlen)
            learnable_audio_len = getattr(config.MOSI.downStream, 'learnable_audio_len', self.alen)
        ...
        # ========== 双向交叉注意力配置 ==========
        self.use_bidirectional_attention = getattr(config.MOSI.downStream, 'use_bidirectional_attention', False)
        self.bidirectional_fusion_mode = getattr(config.MOSI.downStream, 'bidirectional_fusion_mode', 'concat')
        self.bidirectional_fusion_weight = getattr(config.MOSI.downStream, 'bidirectional_fusion_weight', 0.5)
        ...
        # 辅助分类头（可开关）
        self.enable_aux_cls = getattr(config.MOSI.downStream, 'enable_aux_cls', False)
        ...
        self.use_confounder_dict = getattr(config.MOSI.downStream, 'use_confounder_dict', True)  # 该项仅供AtCAF使用
    ...
    def forward(self, text, vision, audio, mode='train', labels=None):
        ...
        if self.atcaf_plugin.enabled:
            ...
            if getattr(self.config.MOSI.downStream, 'atcaf_conservative', False):
