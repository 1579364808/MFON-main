...
def TVA_train_fusion(config, metrics, seed, train_data, valid_data, test_data):
    ...
    # ========== AtCAF K-means 初始化检查 ==========
    need_atcaf_init = config.MOSI.downStream.use_atcaf_plugin and not model.atcaf_plugin.vision_dict.initialized
    ...
    for epoch in range(1, total_epoch + 1):
        ...
        for batch_data in bar:
            try:
                ...
                # AtCAF损失（如果启用）
                if getattr(model.config.MOSI.downStream, 'use_atcaf_plugin', False):
                ...
                # HCL损失（只显示启用的）
                if getattr(model.config.MOSI.downStream, 'enable_hcl', False):
                    lambda1 = getattr(model.config.MOSI.downStream, 'lambda_scl', 0.0)
                    lambda2 = getattr(model.config.MOSI.downStream, 'lambda_iamcl', 0.0)
                    lambda3 = getattr(model.config.MOSI.downStream, 'lambda_iemcl', 0.0)
                ...
                # Focal Loss（如果启用）
                if getattr(model.config.MOSI.downStream, 'enable_aux_cls', False):
            ...
            # ===== 混合对比损失 =====
            if getattr(model.config.MOSI.downStream, 'enable_hcl', False):
                alpha = getattr(model.config.MOSI.downStream, 'contrastive_alpha', 0.5)
                lambda1 = getattr(model.config.MOSI.downStream, 'lambda_scl', 0.0)
                lambda2 = getattr(model.config.MOSI.downStream, 'lambda_iamcl', 0.0)
                lambda3 = getattr(model.config.MOSI.downStream, 'lambda_iemcl', 0.0)
            ...
            if cls_logits is not None:
                ...
                fa = getattr(model.config.MOSI.downStream, 'focal_alpha', None)
                if fa is None:
                    ...
                    alpha_pos = float(fa)
                gamma = getattr(model.config.MOSI.downStream, 'focal_gamma', 2.0)
                focal_w = getattr(model.config.MOSI.downStream, 'focal_weight', 0.1)
            ...
            if torch.is_tensor(atcaf_loss_raw) and atcaf_loss_raw.item() > 0:
                counterfactual_weight = model.config.MOSI.downStream.counterfactual_weight

                # warm-up: 前 atcaf_warmup_epochs 轮，将权重线性从 0 -> counterfactual_weight
                warmup_epochs = getattr(model.config.MOSI.downStream, 'atcaf_warmup_epochs', 0)
        ...
        # 保存当前模型（最后一个模型）
        model_name = f'last_model_epoch_{epoch}_loss_{valid_loss:.4f}.pt'
        last_model_path = os.path.join(model.model_path, model_name)
        torch.save(model.state_dict(), last_model_path)

        print(f"\n💾 模型已保存! Epoch: {epoch}, 验证损失: {valid_loss:.4f}")
        print(f"   模型路径: {last_model_path}")

    # ========== 训练结束，使用最后一个模型进行最终评估 ==========
    print("\n" + "="*50)
    print("🏁 训练完成! 使用最后一个模型进行最终评估...")
    print("="*50)

    if last_model_path and os.path.exists(last_model_path):
        print(f"🔄 正在加载最后一个模型 (第 {total_epoch} 个epoch)...")
        print(f"   模型路径: {last_model_path}")
        ...
        print("\n" + "="*50)
        print("🎉 基于最后一个模型的最终测试结果 🎉")
        print(f"📊 最后一个模型 (第 {total_epoch} 个epoch)")
        ...
        print("❌ 未找到可用的最后模型，直接使用当前模型进行最终测试。")
        print("\n🧪 使用当前模型在测试集上进行最终评估...")
        final_test_results, _ = eval(model, metrics, test_data, device)

        print("\n" + "="*50)
        print("🎉 基于当前模型的最终测试结果 🎉")
        print(f"📊 当前模型 (第 {total_epoch} 个epoch)")
        print(f"  - MAE: {final_test_results['MAE']:.4f}")
        print(f"  - Corr: {final_test_results['Corr']:.4f}")
        print(f"  - Accuracy-7: {final_test_results['Mult_acc_7']:.4f}")
        print(f"  - Accuracy-2 (Has0): {final_test_results['Has0_acc_2']:.4f}")
